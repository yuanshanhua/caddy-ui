...
export const durationSchema = z
  .string()
  .refine(
    (val) => val === "" || /^\d+(\.\d+)?(ns|us|µs|ms|s|m|h)$/.test(val),
...
export const addressSchema = z
  ...
  .refine(
    (val) => /^[^:]+:\d+$/.test(val) || /^:\d+$/.test(val) || /^[^:]+$/.test(val),
...
export const listenAddressSchema = z
  ...
  .refine(
    (val) =>
      /^(:\d+|[\w.-]+:\d+|\d{1,3}(\.\d{1,3}){3}:\d+|\[[\w:]+\]:\d+)$/.test(val),
...
export const positiveIntString = z
  .string()
  .refine(
    (val) => val === "" || (/^\d+$/.test(val) && Number.parseInt(val, 10) > 0),
...
export const httpStatusSchema = z
  .string()
  .refine(
    (val) => {
      if (val === "") return true;
